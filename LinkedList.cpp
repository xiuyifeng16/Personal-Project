...
//In the .cpp, all functions taht use the Comparator will be preceeded by a double header, as such:
template<typename ItemType>
template <typename Comparator>
void LinkedList<ItemType>::bubbleSort(Comparator comp,int*counter)
{
    Node<ItemType> *current = head_;//get headpointer
    Node<ItemType> *current_next = current->getNext();//get next pointer
    for(int i=0;i<size_;i++)
    {
        current = head_;//get headpointer
        current_next = current->getNext();
        while (current != nullptr&&current_next!= nullptr)
        {
            if(comp(*(current_next->getItem()),*(current->getItem())))//if to compare
            {
                ItemType temp=current->getItem();
                current->setItem(current_next->getItem());
                current_next->setItem(temp);//swap the item
                (*counter)++;
            }
            current=current->getNext();
